...
const LEAGUE_ROW_SIDE_PADDING = 16;
const LEAGUE_ROW_LEFT_BORDER = 4;
const LEAGUE_HEADER_LEFT_INSET = LEAGUE_ROW_SIDE_PADDING + LEAGUE_ROW_LEFT_BORDER;
const LEAGUE_ROW_TEMPLATE = '44px minmax(0, 1fr) 64px 56px 12px';
const LEAGUE_TRADE_PLACEHOLDER_WIDTH = 84;
        ...
        <div className="flex items-center w-full">
          <div
            ...
            style={{
              ...
              paddingLeft: LEAGUE_HEADER_LEFT_INSET,
              paddingRight: LEAGUE_ROW_SIDE_PADDING,
            }}
          >
            <div />
            <span className="min-w-0 text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--color-label-tertiary)' }}>Player</span>
            <span className="text-center text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--color-label-tertiary)' }}>Season</span>
            <span className="text-center text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--color-label-tertiary)' }}>Avg/G</span>
            <div />
          </div>
          <div className="shrink-0 ml-3" style={{ width: LEAGUE_TRADE_PLACEHOLDER_WIDTH }} />
        </div>
      </div>
        ...
        POSITION_ORDER.filter(pos => grouped[pos]?.length).map(pos => (
          <div key={pos} className="mb-4">
            ...
            {grouped[pos].map(player => {
              ...
              return (
                <LeagueStyledPlayerRow key={player.id} player={player} nameColumnWidth={sharedNameColumnWidth} onSelect={() => setSelectedPlayerId(player.id)}
                  onTrade={
